...
class PlanningEngine {
    ...
    _decompose(task) {
        ...
        if (/\b(deploy|publish|production)\b/.test(text)) {
            subtasks.push(
                { name: 'install_deps', description: 'Install dependencies', requiredTools: [], expectedResult: 'dependencies installed', dependencies: [] },
                { name: 'build', description: 'Build project', requiredTools: [], expectedResult: 'build successful', dependencies: ['install_deps'] },
                { name: 'test', description: 'Run tests', requiredTools: [], expectedResult: 'tests passing', dependencies: ['build'] },
                { name: 'deploy', description: 'Deploy to target', requiredTools: ['read_file'], expectedResult: 'deployment complete', dependencies: ['test'] },
            ...
            subtasks.push(
                { name: 'identify', description: 'Identify the issue', requiredTools: ['read_file'], expectedResult: 'issue identified', dependencies: [] },
                { name: 'fix', description: 'Apply fix', requiredTools: ['write_file'], expectedResult: 'fix applied', dependencies: ['identify'] },
                { name: 'verify', description: 'Verify the fix', requiredTools: ['read_file'], expectedResult: 'fix verified', dependencies: ['fix'] }
            );
        } else {
            subtasks.push(
